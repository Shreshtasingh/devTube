...
const { createUniqueHandle } = require("@lib/utils") //to create unique handle
// Configure the Google strategy for use by Passport
passport.use(
    new GoogleStrategy( //user credentials
        {
        ...
        async (accessToken, refreshToken, profile, cb) => {
            try {
                ...
                if (!channel) {
                    // Find handles with the same base name as the email username and if found make it unique // creating a username for the channel
                ...
                // Return the channel through the callback cb means call back
                cb(null, channel) //null means no error and channel is the data
            } catch (err) {
                // Handle errors
                cb(err)
            }
...
// Deserialize the user from the session //to read the data 
